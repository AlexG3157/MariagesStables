...
\begin{figure}
      \centering
      \includegraphics[width=7cm]{img/t_fix.png}
      \includegraphics[width=7cm]{img/it_fix.png}
      \caption{Exécution de l'algorithme Gale-Shapley avec 10 parcours.}
      \label{fig:fix}
\end{figure}
...
d'exécution et le nombre d'itérations pour des instances allant de 200 à 2000 étudiants. 
...
L'algorithme a d'abord été évalué sur des matrices de préférences aléatoires et uniformes. 
En fixant initialement le nombre de parcours à $m=10$, nous observons que le temps d'exécution 
et le nombre d'itérations adoptent un comportement quasi-linéaire (Figure \ref{fig:fix}). 
Ce résultat est cohérent avec la complexité théorique en $\mathcal{O}(n \times m \log k)$, 
qui devient $\mathcal{O}(n \log n)$ lorsque $m$ est constante.  

Afin d'étudier le comportement asymptotique de l'algorithme, nous avons ensuite fait varier 
simultanément le nombre d'étudiants et de parcours en fixant $m=n$. Les résultats présentés en 
Figure \ref{fig:moy} confirment un temps d'exécution quadratique. Toutefois, le nombre 
d'itérations croît plus lentement que prévu. Cette observation corrobore les travaux de \cite{wil72}, 
qui démontrent que, sous l'hypothèse de préférences indépendantes et uniformes, le nombre 
moyen de propositions est en $\mathcal{O}(n \log n)$. La divergence observée entre le temps 
d'exécution et le nombre d'itérations s'explique par la phase de pré-calcul des rangs, dont 
la complexité reste structurellement en $\mathcal{O}(n \times m)$ indépendamment de la 
distribution des préférences.  

Finalement, nous avons soumis l'implémentation à un scénario de pire cas (Figure \ref{fig:pire}). 
...
Dans nos tests, cette affectation était possible uniquement pour les $k\geq 5$
...
d'équité (utilité minimale des étudiants) et d'efficacité (somme totale des utilités). 
...
Nous pouvons constater que les deux variantes de l'algorithme de Gale-Shapley aboutissent à la même affectation, 
démontrant que le mariage stable est unique pour cette instance. Le programme linéaire maximisant l'utilité totale 
atteint la meilleure efficacité globale au détriment de la stabilité et de l'équité, puisqu'au moins un étudiant 
obtient son pire choix. À l'inverse, l'approche égalitariste protège les étudiants les plus défavorisés mais 
s'accompagne d'une chute de l'efficacité et de la plus forte instabilité. Dans ce contexte, l'optimisation sous 
contrainte de rang se révèle être un excellent compromis. Elle garantit un niveau de satisfaction minimal et limite 
les paires instables tout en offrant une efficacité globale presque équivalente à celle du mariage stable.
